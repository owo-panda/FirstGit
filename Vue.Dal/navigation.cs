...
namespace Vue.DAL
{
    ...
    public partial class navigation
    {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from vue_navigation");
        ...
        public int Add(Model.navigation model)
        {
            ...
            strSql.Append("insert into vue_navigation(");
        ...
        public bool Update(Model.navigation model)
        {
            ...
            PropertyInfo[] pros = model.GetType().GetProperties();
            List<SqlParameter> paras = new List<SqlParameter>();
            strSql.Append("update vue_navigation set ");
            foreach (PropertyInfo pi in pros)
            {
                //如果不是主键则追加sql字符串
                if (!pi.Name.Equals("id"))
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from vue_navigation ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        public Model.navigation GetModel(int id)
        {
            ...
            strSql.Append(" from vue_navigation");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                    new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM vue_navigation");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from vue_navigation");
            strSql.Append(" where name=@nav_name");
            SqlParameter[] parameters = {
                    new SqlParameter("@nav_name", SqlDbType.NVarChar,50)};
            parameters[0].Value = nav_name;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,parent_id from vue_navigation");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update vue_navigation set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update vue_navigation set name=@new_name");
        ...
        /// 快捷添加系统默认导航
        /// </summary>
        public int Add(string parent_name, string nav_name, string title, string link_url, int sort_id, string action_type)
        {
            //先根据名称查询该父ID
            StringBuilder strSql1 = new StringBuilder();
            strSql1.Append("select top 1 id from vue_navigation");
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into vue_navigation(");
            strSql.Append("parent_id,nav_type,name,title,link_url,sort_id,action_type,is_lock,is_sys)");
            strSql.Append(" values (");
            strSql.Append("@parent_id,@nav_type,@name,@title,@link_url,@sort_id,@action_type,@is_lock,@is_sys)");
            strSql.Append(";select @@IDENTITY");
            SqlParameter[] parameters = {
            ...
            parameters[0].Value = parent_id;
            parameters[1].Value = VueEnums.NavigationEnum.System.ToString();
            parameters[2].Value = nav_name;
            parameters[3].Value = title;
            parameters[4].Value = link_url;
            parameters[5].Value = sort_id;
            parameters[6].Value = action_type;
        ...
        /// 快捷添加系统默认导航，带事务
        /// </summary>
        public int Add(SqlConnection conn, SqlTransaction trans, string parent_name, string nav_name, string title, string link_url, int sort_id, string action_type)
        {
            //先根据名称查询该父ID
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from vue_navigation");
            ...
            return Add(conn, trans, parent_id, nav_name, title, link_url, sort_id, action_type);
        }

        /// <summary>
        /// 快捷添加系统默认导航，带事务
        /// </summary>
        public int Add(SqlConnection conn, SqlTransaction trans, int parent_id, string nav_name, string title, string link_url, int sort_id, string action_type)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into vue_navigation(");
            strSql.Append("parent_id,nav_type,name,title,link_url,sort_id,action_type,is_lock,is_sys)");
